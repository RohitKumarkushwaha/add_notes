...
const AllNotes = ({ notes, setNotes }) => {
    ...
    return (
        <>
            <h1 className='text-[42px] font-extrabold text-blue-600'>NOTES</h1>
            <input value={query} onChange={(e) => searchNote(e.target.value)} className='text-center focus:outline-none p-2 rounded-full bg-[#EFF6FF] w-full shadow-lg' placeholder='Search' type="text" />
            <p className='mt-4 text-blue-600'>{query !== '' ? filteredNotes.length : notes.length} Notes</p>
            <div className='md:overflow-y-scroll scroll md:max-h-[70vh] flex flex-col gap-4 pt-4'>
                {filteredNotes.length < 1 ? (
                    <p className='text-center text-xl font-semibold pt-10 text-blue-600'>No Notes</p>
                ) : (
                    filteredNotes?.map(note => (
                        <div key={note.id} className='md:w-[450px] w-full h-[140px] hover:bg-[#CFE4FF] transition-all duration-300 flex flex-col gap-2 p-6 pt-4 rounded-xl bg-[#EFF6FF] shadow-lg'>
                            ...
                            <div className='flex relative px-1 -bottom-2 justify-between'>
                                <span className='text-sm text-gray-600'>{timeStamp(note.timestamp)}</span>
                                <span onClick={() => deleteNote(note.id)}><RiDeleteBin6Fill size={20} className='cursor-pointer text-red-500' /></span>
                            </div>
                        </div>
                    ))
                )}
                <div className='flex items-center justify-center gap-8'>
                    <span onClick={() => handlePrevPage()} className='p-2 cursor-pointer rounded-full border border-gray-400 hover:bg-gray-200 transition duration-200'><RiArrowLeftSLine /></span>
                    <span className='text-blue-600'>Page {filteredNotes.length < 1 ? '0' : currentPage} of {query !== '' ? filteredTotalPages : totalPages}</span>
                    <span onClick={() => handleNextPage()} className='p-2 cursor-pointer rounded-full border border-gray-400 hover:bg-gray-200 transition duration-200'><RiArrowRightSLine /></span>
                </div>
            </div>
        </>
    )
